...
- [ ] Load `/` at 1200px wide → the image runs nearly full bleed with rounded corners, and the headline sits in a dark carve that hugs it and bleeds off the left edge → AC-2
- [ ] Load `/` → the headline is in Cormorant Garamond and the text can be selected with the cursor (it is real text, not an image) → AC-3
- [ ] **Not yet exercised.** Disable JavaScript, load `/` → the carve is fully drawn and the image is visible immediately. Nothing about the shape depends on JavaScript → AC-2, AC-10
- [ ] Change `heroHeadline` in the Studio to a sentence long enough to wrap onto three lines, publish, reload → each line gets its own dark backing, stepping in as the lines get shorter, with no gap of photograph showing between lines → AC-4
- [ ] Load `/` and look at the nav → its links and CTA stay readable against the image, thanks to the scrim behind the bar → AC-8
...
- [ ] **Not yet exercised.** Turn on Reduce Motion in the operating system, reload `/` → the headline is visible immediately with no reveal animation at all → AC-9
